...
* **Model Discrimination & Rank-Ordering (ROC-AUC & Gini):** The **Custom Random Forest (OOP)** achieved the highest overall discriminatory power on the holdout test set (**ROC-AUC: 0.7587**, **Gini: 0.5174**). This demonstrates that ensembling non-linear decision trees significantly enhances the model's ability to rank-order borrower risk compared to linear baselines.
* **Addressing Class Imbalance (PR-AUC vs. Accuracy):** Because defaults comprise only ~22% of the portfolio, raw accuracy plateaus artificially around **81.6%–81.9%** across all architectures (a naive model predicting zero defaults achieves ~78%). The **Pruned Decision Tree (Entropy, $\alpha = 0.0007$)** captured the highest **PR-AUC of 0.5376**. Prioritizing PR-AUC over accuracy ensures the model reliably identifies defaulting borrowers, directly minimizing institutional exposure to unrecoverable charge-offs (False Negatives).
* **Risk Population Separation (K-S Statistic):** The **Logistic Regression (Degree 2 Map)** produced the highest **K-S statistic at 38.52%** (5-Fold CV Mean). A K-S statistic approaching 40% reflects strong separation between the cumulative probability distributions of defaulters and non-defaulters, offering a reliable decision boundary for setting credit score cutoffs in automated underwriting workflows.
